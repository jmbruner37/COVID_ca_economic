...
connection_string = "postgres:test123@localhost:5432/covid_cases3" #postgres:insert password
...
Covid_cases2 = Base.classes.covid_cases2
Unemployment_numbers2 = Base.classes.unemployment_numbers2
Home_price2 = Base.classes.home_price2
...
def covid2():
    ...
    # Query all the data
    # results = session.query(Covid_cases.county, Covid_cases.month, Covid_cases.new_cases, Covid_cases.new_deaths).all()
    results = session.query(Covid_cases2.county, Covid_cases2.date, Covid_cases2.total_case_count).all()
...
@app.route("/unemployment_numbers2")
def unemployment2():
    # Create our session (link) from Python to the DB
    session = Session(engine)
...
# Query all the data 
    # results = session.query(Unemployment_numbers.county, Unemployment_numbers.month, Unemployment_numbers.labor_force, Unemployment_numbers.employment, Unemployment_numbers.unemployment, Unemployment_numbers.unemployment_rate).all()
    results = session.query(Unemployment_numbers2.county, Unemployment_numbers2.date, Unemployment_numbers2.unemployment).all()
...
# Query all the data 
    # results = session.query(Home_price.county, Home_price.month, Home_price.price).all()
    results = session.query(Home_price2.county, Home_price2.date, Home_price2.price).all()
