...
d3.csv("final_joined_data.csv").then(function (joinedData) {
  ...
  var xTimeScale = d3.scaleLinear().domain(d3.extent(joinedData, function (d) {
    return d.month;
  })).range([0, width]);
  ...
  var line1 = d3.line().x(function (d) {
    ...
    return yLinearScale1(d.new_deaths);
  });
  var line2 = d3.line().x(function (d) {
    return xTimeScale(d.month);
  }).y(function (d) {
    return yLinearScale2(d.unemployment_rate);
  ...
  chartGroup.append("text").attr("transform", "translate(".concat(width / 2, ", ").concat(height + margin.top + 20, ")")).classed("new_deaths-text text", true).text("New COVID Deaths_Unemployment Rates");
  chartGroup.append("text").attr("transform", "translate(".concat(width / 2, ", ").concat(height + margin.bottom + 37, ")")).classed("price-text text", true).text("Unemployment Rates");
